...
This Software Guidebook follows [Simon Brown's methodology](https://softwarearchitecturefordevelopers.com/) from "Software Architecture for Developers Vol. 2". It provides comprehensive documentation of the knowledge platform application's architecture, design decisions, and operational aspects.

The architecture of the application is designed in a modular fashion. Meaning that a specific instance of the application basically needs two non-modular components: a domain specific graph model and domain specific data sources. These differences are driven by the context for which a specific application is used. Because of this modular architecture, we have one general software guidebook, describing the modular components in a generic way. And we have a software guidebook per specific instance of the application, describing de specific context and implementations of the general components. If there are no specificities, the application specific guidebook refers to the general guidebook. 
...
**Disclaimer**: this document is partly generated by artificial intelligence. This follows a design pattern: **Chapter 1. Context** and **Chapter 2. Functional Overview** are written by human stakeholders as input for the GenAI. Then, a lead developer specialised in agentic AI, uses this input and the codebase to generate and redact a first version of the software guidebook. After that this redacted, generated version is transferred to the development team for redaction. After this stage, the software guidebook is accepted for the current state of the application and automatically updated with GenAI with the same human in the loop principles.
