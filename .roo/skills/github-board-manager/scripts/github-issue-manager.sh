...
load_config() {
    ...
    # Find project root by looking for .roo/skills directory
    local script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    PROJECT_ROOT="$(cd "$script_dir" && while [[ ! -d ".roo/skills" && "$(pwd)" != "/" ]]; do cd ..; done && pwd)"
    
    if [[ ! -d "$PROJECT_ROOT/.roo/skills" ]]; then
        output_error "Could not find project root with .roo/skills directory"
    fi
    
    CONFIG_FILE="$PROJECT_ROOT/.roo/skills/core-config.yaml"
...
create_epic_issue() {
    ...
    ensure_label "epic:$epic_slug" "Epic issues" "1e3a8a"
...
create_feature_issue() {
    ...
    if [ -z "$title" ] || [ -z "$body" ] || [ -z "$feature_slug" ]; then
        output_error "Usage: create-feature <title> <body> <feature_slug> [--epic <epic_num> <epic_slug>]"
    fi
...
Parent Epic: $epic_slug (#$epic_num)"
    ...
    # Create labels
    local labels="feature"
    if [ -n "$epic_slug" ]; then
        ensure_label "epic:$epic_slug" "Epic $epic_num stories" "1e3a8a"
        labels="$labels,epic:$epic_slug"
    fi
    ensure_label "feature:$feature_slug" "feature $feature_slug" "9017ca"
    labels="$labels,feature:$feature_slug"
...
create_task_issue() {
    ...
    if [ -z "$title" ] || [ -z "$body" ]; then
        output_error "Usage: create-task <title> <body> [--feature <feature_num> <feature_slug>] [--epic-slug <epic_slug>]"
    fi
    
    # If feature_num is provided, feature_slug must also be provided (and vice versa)
    if [ -n "$feature_num" ] && [ -z "$feature_slug" ]; then
        output_error "When providing --feature, both <feature_num> and <feature_slug> are required"
    fi
...
Epic Slug: $epic_slug"
    ...
    if [ -n "$feature_slug" ]; then
        ensure_label "feature:$feature_slug" "feature $feature_slug tasks" "9017ca"
        labels="$labels,feature:$feature_slug"
    fi
...
*Migrated from: $filename*"
            ...
            if echo "$result" | jq -e '.feature_number' >/dev/null 2>&1; then
                ...
                # Add migration label
                gh issue edit "$feature_issue_num" --add-label "migrated" --repo "$REPO" >/dev/null 2>&1
...
# Syntax: create-feature <title> <body> <feature_slug> [--epic <epic_num> <epic_slug>]
parse_create_feature_args() {
    ...
    # Call create_feature_issue with parameter order: title, body, feature_slug, epic_num, epic_slug
    create_feature_issue "$title" "$body" "$feature_slug" "$epic_num" "$epic_slug"
}

# Parse create-task arguments
# Syntax: create-task <title> <body> [--feature <feature_num> <feature_slug>] [--epic-slug <epic_slug>]
parse_create_task_args() {
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            --feature)
                if [ $# -lt 3 ]; then
                    output_error "--feature requires two arguments: <feature_num> <feature_slug>"
                fi
                feature_num="$2"
                feature_slug="$3"
                shift 3
