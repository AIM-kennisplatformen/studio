...
import { applyFcoseLayout } from './utils/cytoscapeLayout';
import { calculateNodeDistances } from './utils/graphTraversal';
import { getEdgeHandles, getEntityStyle } from './lib/graphUtils';
import { transformKnowledgeGraph } from './data/knowledgeGraphData';
import { useAtom } from 'jotai';
import { graphStructureAtom, selectedNodeAtom } from './data/atoms';

function Graph() {
  const { setViewport, getViewport, updateNodeInternals } = useReactFlow();
  const [structure, setStructure] = useAtom(graphStructureAtom);
  ...
  useEffect(() => {
    ...
    setStructure({ nodes: nodesWithPositions, edges: newEdges });
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      const distances = calculateNodeDistances(selectedNode.id, structure.nodes, structure.edges);
